...
int main(int argc, char* argv[])
{
	...
	while(1) {
		...
		while (res == 0) {
			res = hid_read_timeout(handle, rbuf, sizeof(rbuf), 1);
			/* if (res == 0) */
			/* 	printf("waiting...\n"); */
			/* if (res < 0) */
		...
		unsigned int elapsed_ms = elapsed.tv_usec / 1000;

		printf(" Report: 0x%02X, A1: %d, A2: %d, A3: %d, A4: %d, B1: 0x%02X, B2: 0x%02X",
		       rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6]);
		printf(" Lapsed: %4d.%03d s",
